...
function getZoneFromLabel(label: string): string {
  ...
  if (base.startsWith("section_numbers")) return "Chiffres";
  if (base.startsWith("section_projects")) return "Projets";
  if (base.startsWith("section_contact")) return "Contact";
  if (base.startsWith("language_toggle")) return "Langue";
  if (base.startsWith("project_") || base.startsWith("page_projet")) return "Projets";
  return "Autres";
}
...
/** Libellé lisible pour le backoffice : précise FR/EN (langue du site) et les actions (ex. "vers FR"). */
function formatDisplayLabel(cleanedLabel: string): string {
  if (cleanedLabel === "language_toggle_fr") return "Changement langue → FR (vers français)";
  if (cleanedLabel === "language_toggle_en") return "Changement langue → EN (vers anglais)";
  const navNames: Record<string, string> = {
    nav_home: "Nav: Accueil",
    nav_about: "Nav: À propos",
    nav_numbers: "Nav: Chiffres",
    nav_projects: "Nav: Projets",
    nav_contact: "Nav: Contact",
    nav_burger: "Nav: Menu burger",
  };
  return navNames[cleanedLabel] ?? cleanedLabel;
}
...
export default function AnalyticsDashboard() {
  ...
  const fetchDailyStats = async () => {
    try {
      setError(null);
      const res = await fetch("/api/analytics/daily?limit=365", {
        method: "GET",
        credentials: "include",
  ...
  const handleAggregate = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Erreur lors de l'agrégation");
      const result = await res.json();
      setSuccessMessage(`✅ Succès pour le ${result.result?.date ?? "jour"}`);
      await Promise.all([fetchDailyStats(), fetchMonthlyStats(), fetchYearlyStats()]);
    } catch (err) {
  ...
  const filteredDailyData = useMemo(() => {
    ...
    return dailyStats.filter((s) => s.date === selectedDate);
  }, [dailyStats, selectedDate]);

  const availableMonths = useMemo(() => {
    const set = new Set<string>();
    monthlyStats.forEach((m) => set.add(`${m.year}-${String(m.month).padStart(2, "0")}`));
  ...
  const currentData: NormalizedStat[] = useMemo(() => {
    if (period === "daily") {
      return filteredDailyData.map((d) => ({
        ...
        clicks: (d.clicks as Record<string, number>) || {},
    ...
    if (period === "monthly") {
      return filteredMonthlyStats.map((m) => ({
        ...
        clicks: (m.clicks as Record<string, number>) || {},
    ...
    return filteredYearlyStats.map((y) => ({
      ...
      clicks: (y.clicks as Record<string, number>) || {},
  ...
  const comparisonChartData = useMemo(() => {
    return [...currentData].reverse().map((stat) => {
      ...
      const mobileClicks = Object.entries(clicks)
        .filter(([label]) => label.toLowerCase().includes("mobile"))
        .reduce((sum, [, count]) => sum + count, 0);
      const desktopClicks = Object.entries(clicks)
        .filter(([label]) => !label.toLowerCase().includes("mobile"))
        .reduce((sum, [, count]) => sum + count, 0);
      return {
        name: stat.date,
  ...
  const totalPageViews = currentData.reduce((s, i) => s + i.pageViews, 0);
  const totalVisitors = currentData.reduce((s, i) => s + i.uniqueVisitors, 0);
  ...
  const { desktopFR, desktopEN, mobileFR, mobileEN } = useMemo(() => {
    ...
    currentData.forEach((item) => {
      Object.entries(item.clicks || {}).forEach(([label, count]) => {
        ...
        target[zone][cleanedLabel] = (target[zone][cleanedLabel] || 0) + count;
      });
    });
    return {
  ...
  const zoneChartData = useMemo(() => {
    ...
    currentData.forEach((item) => {
      Object.entries(item.clicks || {}).forEach(([label, count]) => {
        const zone = getZoneFromLabel(label);
        byZone[zone] = (byZone[zone] || 0) + count;
      });
    });
  ...
  const zoneChartDataWithColors = useMemo(
    () => zoneChartData.map((d, i) => ({ ...d, fill: ZONE_COLORS[i % ZONE_COLORS.length] })),
    [zoneChartData]
  );
    ...
    <div className={styles.analyticsContainer}>
      <div className={styles.header}>
        ...
        <p className={styles.headerHint}>
          Répartition par langue du site au moment du clic (version FR = visiteur sur le site en français, version EN = visiteur sur le site en anglais) et par type d’appareil (Desktop / Mobile).
        </p>
        <div className={styles.controls}>
          ...
          {period === "daily" && (
            ...
            >
              ...
              {dailyStats.map((stat) => (
                <option key={stat.id} value={stat.date}>
                  {stat.date}
                </option>
              ))}
            </select>
      ...
      {zoneChartDataWithColors.length > 0 && (
        <div className={styles.chartSection}>
          ...
          <p className={styles.chartDescription}>
            Répartition des clics : navigation, sections, footer, etc.
          </p>
          <div style={{ width: "100%", height: 320 }}>
            <ResponsiveContainer>
              <BarChart data={zoneChartDataWithColors} layout="vertical" margin={{ left: 80 }}>
                ...
                <Tooltip formatter={(value: unknown) => (typeof value === "number" ? [value, "clics"] : [])} />
      ...
      {period === "monthly" && filteredMonthlyStats.length > 0 && (
        <div className={styles.monthlyDetailSection}>
          ...
          {filteredMonthlyStats.map((month) => {
            ...
            const dailyStatsArr = (month.dailyStats || []) as DailyStatSub[];
            return (
              <div key={month.id} className={styles.monthDetail}>
                <div
                ...
                >
                  <h4>
                    ...
                    <small>
                      {" "}
                      (total : {month.pageViews} vues • {month.uniqueVisitors} uniques)
                    </small>
                  </h4>
                  <span className={styles.toggleIcon}>{isOpen ? "▲" : "▼"}</span>
                </div>
                <div className={styles.accordionContent} aria-hidden={!isOpen}>
                  {dailyStatsArr.length > 0 ? (
      ...
      <div className={styles.platformComparison}>
        <div className={styles.platformSection}>
          ...
          >
            <h3 className={styles.platformTitle}>
              💻 Desktop — Clics sur la version française du site (
              {Object.values(desktopFR).reduce(
                      ...
                      {Object.entries(actions)
                        ...
                        .map(([action, count]) => (
                          <div key={action} className={styles.clickItem}>
                            <span className={styles.clickLabel}>{formatDisplayLabel(action)}</span>
        ...
        <div className={styles.platformSection}>
          ...
          >
            <h3 className={styles.platformTitle}>
              💻 Desktop — Clics sur la version anglaise du site (
              {Object.values(desktopEN).reduce(
                (sum, cat) => sum + Object.values(cat).reduce((a, b) => a + b, 0),
                0
              )}{" "}
              clics)
        ...
        <div className={styles.platformSection}>
          ...
          >
            <h3 className={styles.platformTitle}>
              📱 Mobile — Clics sur la version française du site (
              {Object.values(mobileFR).reduce(
                (sum, cat) => sum + Object.values(cat).reduce((a, b) => a + b, 0),
                0
              )}{" "}
              clics)
        ...
        <div className={styles.platformSection}>
          ...
          >
            <h3 className={styles.platformTitle}>
              📱 Mobile — Clics sur la version anglaise du site (
              {Object.values(mobileEN).reduce(
                (sum, cat) => sum + Object.values(cat).reduce((a, b) => a + b, 0),
                0
              )}{" "}
              clics)
